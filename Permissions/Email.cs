...
namespace SitePermissions
{
    public static class Email
    {
        ...
        public static async Task<List<Tuple<Microsoft.Graph.User, bool>>> InformOwners(ICollection<Site> sites, GraphServiceClient graphAPIAuth,  ILogger log)
        {
            ...
            var siteOwnersInfo = new List<Tuple<Microsoft.Graph.User, string, string>>();
            ...
            foreach (var ownerInfo in siteOwnersInfo)
            {
                var result = await SendMisconfiguredEmail(ownerInfo.Item1.DisplayName, ownerInfo.Item1.Mail, ownerInfo.Item2, ownerInfo.Item3, log);
                results.Add(new Tuple<Microsoft.Graph.User, bool>(ownerInfo.Item1, result));
            }
            return results;
        }
